...
var browser = {
  versions: (function () {
    ...
    return {
      ...
      iPhone:
        u.indexOf('iPhone') > -1 || u.indexOf('Mac') > -1, //是否为iPhone或者安卓QQ浏览器
...
var pJS = function (tag_id, params) {
  ...
  pJS.fn.retinaInit = function () {
    ...
    pJS.canvas.w =
      pJS.canvas.el.offsetWidth * pJS.canvas.pxratio;
    pJS.canvas.h =
      pJS.canvas.el.offsetHeight * pJS.canvas.pxratio;
  ...
  pJS.fn.particle.prototype.draw = function () {
    ...
    if (pJS.particles.shape.stroke.width > 0) {
      pJS.canvas.ctx.strokeStyle =
        pJS.particles.shape.stroke.color;
      pJS.canvas.ctx.lineWidth =
        pJS.particles.shape.stroke.width;
    ...
    for (var i = 0; i < pJS.particles.array.length; i++) {
      ...
      if (pJS.particles.size.anim.enable) {
        if (p.size_status == true) {
          ...
          if (
            p.radius <= pJS.particles.size.anim.size_min
          )
            p.size_status = true;
          p.radius -= p.vs;
        }
  ...
  pJS.fn.interact.attractParticles = function (p1, p2) {
    ...
    if (dist <= pJS.particles.line_linked.distance) {
      var ax =
        dx /
        (pJS.particles.move.attract.rotateX * 1000),
        ay =
          dy /
          (pJS.particles.move.attract.rotateY * 1000);
  ...
  pJS.fn.modes.bubbleParticle = function (p) {
    ...
    if (
      ...
      if (pJS.tmp.bubble_clicking) {
        var dx_mouse =
          p.x - pJS.interactivity.mouse.click_pos_x,
          dy_mouse =
            p.y - pJS.interactivity.mouse.click_pos_y,
          dist_mouse = Math.sqrt(
            dx_mouse * dx_mouse + dy_mouse * dy_mouse
          ),
          time_spent =
  ...
  pJS.fn.modes.grabParticle = function (p) {
    if (
      ...
      if (
        dist_mouse <= pJS.interactivity.modes.grab.distance
      ) {
        var opacity_line =
          ...
          if (pJS.interactivity.events.onclick.enable) {
            ...
            pJS.interactivity.events.onclick.mode
              ...
              case 'repulse':
                ...
                setTimeout(function () {
                  pJS.tmp.repulse_clicking = false;
                }, pJS.interactivity.modes.repulse
                  .duration * 1000);
                break;
            }
          }
  ...
  xhr.onreadystatechange = function (data) {
    if (xhr.readyState == 4) {
      if (xhr.status == 200) {
        ...
        console.log(
          'Error pJS - XMLHttpRequest status: ' +
          xhr.status
        );
